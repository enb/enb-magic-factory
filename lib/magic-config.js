 ...
 * @constructor
 */
var MagicConfig = function (requiredTargets) {
...
MagicConfig.prototype.isRequiredNode = function (node) {
    return this.isRequiredTarget(node);
};
...
MagicConfig.prototype.isRequiredTarget = function (target) {
    ...
    var requiredTargets = this._requiredTargets;
    var need = false;
    ...
    requiredTargets.forEach(function (requiredTarget) {
        ...
        var splitedArg = requiredTarget.split(path.sep);

        if ((splitedArg.length === splitedTarget.length && target === requiredTarget) ||
            (splitedArg.length < splitedTarget.length &&
                (requiredTarget === splitedTarget.splice(0, splitedArg.length).join(path.sep))
            )
        ) {
            need = true;
        }
...
/**
 * Возвращает список целей для сборки make-платформой.
 *
 * @returns {String[]}
 */
MagicConfig.prototype.toBuild = function () {
    var targets = {};
    ...
    return [].concat(Object.keys(nodes), Object.keys(targets));
};
