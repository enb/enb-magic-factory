var path = require('path'),
    ...
    looksLikeWildcard = require('./utils').looksLikeWildcard;
...
MagicConfig.prototype.isRequiredNode = function (node) {
    return this._isRequiredTarget(node, function (required, study, splitR, splitS) {
        ...
        return (splitR.length < splitS.length && cmp(required, splitS.slice(0, splitR.length).join(path.sep))) ||
            // Если ожидаемая цель является частью проверяемой ноды
            (splitR.length > splitS.length && cmp(splitR.slice(0, splitS.length).join(path.sep), study));
    });
};
...
MagicConfig.prototype._isRequiredTarget = function (target, f) {
    ...
    return requiredTargets.some(function (requiredTarget) {
        ...
        if (
            ...
            (splitedRequiredTarget.length === splitedTarget.length && cmp(requiredTarget, target)) ||
            // Специальное условие
...
function cmp(required, study) {
    return required === study ||
        looksLikeWildcard(required) && minimatch(study, required);
}
