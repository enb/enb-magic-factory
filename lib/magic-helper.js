...
/**
 * Возвращает канал связи данного хэлпера.
 *
...
MagicHelper.prototype._configureTask = function () {
    ...
    projectConfig.task(taskName, function (task) {
        ...
        return vow.all(this._chains.map(function (callback) {
                return callback.apply(this, [magicConfig]);
            }))
            .then(function () {
                var nodes = magicConfig.getRegisteredNodes();
                var targets = magicConfig.getRegisteredTargets();

                channel.emit('prebuild', nodes, targets);
...
MagicHelper.prototype._buildTargets = function (targets) {
    ...
    return makePlatform.init(root)
        ...
        .then(function () {
            task.log('building finished - ' + colors.red((new Date() - startTime) + 'ms'));
