 ...
 *
 * Название режима задаётся через переменную окружения `process.env.ENB_MAGIC_MODE`.
        ...
        return vow.all(_this._prebuilds.map(function (callback) {
            ...
            .then(function () {
                var registeredNodes = magicConfig.getRegisteredNodes(),
                    ...
                    nodesToBuild = {},
                    targetsToBuild = {},
                    mode = process.env.ENB_MAGIC_MODE || 'full';
                ...
                args.forEach(function (arg) {
                    ...
                    if (mode === 'full' && magicConfig.isRequiredNode(arg)) {
                ...
                return _this._buildTargets(
                    Object.keys(toBuild),
                    Object.keys(nodesToBuild), Object.keys(targetsToBuild)
                );
            });
    });
    ...
    return makePlatform.init(root)
        ...
        .then(function () {
            if (useCache) {
                ...
                makePlatform._cacheStorage = undefined; // Не даём очистить закэшированные данные основной
                                                        // make-платформы, т.к. они могут понадобится
                                                        // для остальных тасков
            }

            makePlatform.destruct();

            channel.emit('build', toBuild);
